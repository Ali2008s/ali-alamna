...
DateTime? _parseFlexibleDate(String input) {
  ...
  String clean = input
      .replaceAll(RegExp(r'(\d+)(st|nd|rd|th)', caseSensitive: false), r'\1')
...
PlayVideoFrom getVideoPlatform({required String type, required String videoURL}) {
  ...
  final Map<String, String> defaultHeaders = {
    'Referer': DOMAIN_URL,
    'User-Agent':
        'Mozilla/5.0 (Linux; Android 11; TV) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36',
    'Accept': '*/*',
  ...
  try {
    ...
    if ((type.toLowerCase() == PlayerTypes.local.toLowerCase() ||
            type.toLowerCase() == PlayerTypes.url.toLowerCase()) &&
...
String formatEpisodeDuration(String raw) {
  if (raw.trim().isEmpty) return '';
  final parts = raw.split(':');
  if (parts.length < 2) return '';
  final h = parts[0];
  final m = parts[1];
  if (h == '00') return '${int.tryParse(m) ?? 0}m';
