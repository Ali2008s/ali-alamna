...
import 'package:streamit_laravel/screens/matches/matches_screen.dart';
...
class DashboardController extends GetxController {
  ...
  RxList<BottomBarItem> bottomNavItems = [
    ...
    /*BottomBarItem(
        ...
        activeIcon: Icons.app_shortcut_rounded,
        type: BottomItem.shorts,
        focusNode: FocusNode(),
        screen: SizedBox()),*/
    BottomBarItem(
  ...
  void navigateToContentBanner() {
    ...
    final isRegistered =
        Get.isRegistered<ContentListController>(tag: controllerTag);

    if (isRegistered) {
      final contentListController =
          Get.find<ContentListController>(tag: controllerTag);
      ...
      if (Get.isRegistered<ContentListController>(tag: controllerTag)) {
        try {
          final contentListController =
              Get.find<ContentListController>(tag: controllerTag);
          if (contentListController.sliderController.bannerList.isNotEmpty) {
            final canRequest = contentListController
                .sliderController.sliderFocus.canRequestFocus;
            if (canRequest) {
              contentListController.sliderController.sliderFocus.requestFocus();
            } else {
              Future.delayed(Duration(milliseconds: 50), () {
                contentListController.sliderController.sliderFocus
                    .requestFocus();
              });
            }
  ...
  Future<void> onBottomTabChange(BottomItem type) async {
    ...
    try {
      ...
      if ((bottomNavItems[selectedBottomNavIndex.value].type ==
                  BottomItem.home ||
              bottomNavItems[selectedBottomNavIndex.value].type ==
                  BottomItem.movies) &&
          type == BottomItem.search) {
      ...
      switch (type) {
        ...
        case BottomItem.matches:
          ...
          break;
        case BottomItem.shorts:
          newScreen = ShortsScreen();
          break;
