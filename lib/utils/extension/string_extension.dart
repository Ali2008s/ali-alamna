...
extension StrExt on String {
  ...
  String getContentAccess({int requiredPlanLevel = 0}) {
    if (this == MovieAccess.freeAccess || requiredPlanLevel == 0) {
      ...
      return MovieAccess.paidAccess;
    } else if (this == MovieAccess.payPerView) {
      return MovieAccess.payPerView;
    }

    return MovieAccess.freeAccess;
  }

  String getUrlTypeFromUrl([String? urlType]) {
    if (urlType?.isNotEmpty ?? false) {
      return urlType!;
    }
    // Auto-detect URL type from URL
    if (contains('youtube.com') || contains('youtu.be')) {
      return 'youtube';
    } else if (contains('vimeo.com')) {
      return 'vimeo';
    } else if (contains('.m3u8')) {
      return 'hls';
    }
    return 'normal';
  }
